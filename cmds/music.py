...
class Music(commands.Cog):
    ...
    async def create_player(ctx: commands.Context):
        ...
        if not ctx.author.voice or not ctx.author.voice.channel:
            ...
            if voice_client is not None:
                raise commands.CommandInvokeError("❌ 請先加入機器人所在的語音頻道!")

            # Otherwise, tell them to join any voice channel to begin playing music.
            raise commands.CommandInvokeError("❌ 請先加入語音頻道!")
        ...
        if voice_client is None:
            ...
            if voice_channel.user_limit > 0:
                ...
                if (
                    ...
                    raise commands.CommandInvokeError("❌ 語音頻道已滿!")
    ...
    async def on_track_start(self, event: TrackStartEvent):
        ...
        try:
            ...
            lyrics = pylrc.parse(lrc_text) if lrc_text else None
        except:
            lyrics = None

        embed = discord.Embed(
            title=f"🎤 歌詞：{player.current.title}", colour=discord.Color.blurple()
        )
        embed.description = "*載入中...*" if lyrics is None else "歌詞已就緒"
        msg = await channel.send(embed=embed)

        prev = self.lyrics_tasks.get(player.guild_id)
        if prev and not prev.done():
            prev.cancel()

        task = asyncio.create_task(self._lyrics_updater(player, lyrics, msg))
        self.lyrics_tasks[player.guild_id] = task
        ...
        if channel:
            await channel.send(f"▶️ 現在播放: {event.track.title}")
    ...
    async def resume(self, ctx):
        ...
        if player.is_playing and not player.paused:
            return await ctx.send("❌ 目前沒有已暫停的歌曲")
    ...
    async def show(self, ctx):
        ...
        if len(player.queue) == 0:
            return await ctx.send("❌ 佇列是空的")

        queue_titles = [track.title for track in player.queue]
        title = "\n".join(f"- {title}" for title in queue_titles)

        embed = discord.Embed(
            title="佇列", description=title, color=discord.Color.blue()
        )
        await ctx.send(embed=embed)
